...
import { ADMIN, CUSTOMER, DRIVER, IMAGE_DONE, IMAGE_WARN, STAF, SUPER } from 'src/shared/constants';
...
export class SignInComponent implements OnInit {
  ...
  Login() {
    ...
    this.accountService.login({ email, password }).subscribe(user => {
      if (user && user.UserId && user.UserStatus != 'Account Locked' && user.UserStatus !== 'Deactivated') {
        this.beep();
        this.error = '';
        ...
        let err: any = user;
        this.error = err + '. , Or contact us if you did not get the mail.'|| 'Your account is '+user.UserStatus || 'your email or password is incorrect';
        this.showLoader = false;
      }
    });
  }
  ...
  sendLink() {
    ...
    this.accountService.generateToken({ Email: this.myemail }).subscribe(data => {
      if (data.UserToken) {
        ...
        this.sendAltraEmail(this.myemail, EmailHelper.getResetPasswordCustomer(data.Name, link, this.config.Name), 'Password reset link', this.config.Email);
